...
iredapd_config()
{
    ...
    if [ X"${BACKEND}" == X'OPENLDAP' ]; then
        ...
        perl -pi -e 's#^(plugins).*#${1} = ["reject_null_sender", "amavisd_wblist", "ldap_maillist_access_policy"]#' settings.py
        ...
        perl -pi -e 's#^(plugins).*#${1} = ["reject_null_sender", "amavisd_wblist", "sql_alias_access_policy"]#' settings.py
    fi
